...
internal class ThrottleBrakeImplTest {
    ...
    fun `parking brake should be activated`() {
        actionMemProp?.setter?.call(ThrottleBrakePedal.ACTION_PARKING_BRAKE)
    ...
    fun `parking brake should be deactivated`() {
        actionMemProp?.setter?.call(ThrottleBrakePedal.ACTION_BRAKING_STILL)
        valueMemProp?.setter?.call(ThrottleBrakeImpl, 100)
    ...
    fun `handbrake should be activated`() {
        actionMemProp?.setter?.call(ThrottleBrakePedal.ACTION_HANDBRAKE)
        valueMemProp?.setter?.call(ThrottleBrakeImpl, 100)
    ...
    fun `is moving forward`() {
        actionMemProp?.setter?.call(ThrottleBrakePedal.ACTION_MOVE_FORWARD)
        assertThat(ThrottleBrakeImpl.isMovingForward).isTrue()
    }
    @Test
    fun `is not moving forward`() {
        actionMemProp?.setter?.call(ThrottleBrakePedal.ACTION_MOVE_BACKWARD)
    ...
    @Test
    fun `is neutral`() {
        actionMemProp?.setter?.call(ThrottleBrakePedal.ACTION_NEUTRAL)
    ...
    fun `neutral motion state`() {
        ...
        assertThat(ThrottleBrakeImpl.motionState)
            .isEqualTo(ThrottleBrakePedal.ACTION_NEUTRAL)
            .isEqualTo("neutral")
    }
    @Test
    fun `braking still motion state`() {
        `is braking still`()
        assertThat(ThrottleBrakeImpl.motionState)
            .isEqualTo(ThrottleBrakePedal.ACTION_BRAKING_STILL)
    ...
    fun `parking brake motion state`() {
        ...
        assertThat(ThrottleBrakeImpl.motionState)
            .isEqualTo(ThrottleBrakePedal.ACTION_PARKING_BRAKE)
    ...
    fun `forward motion state`() {
        `is moving forward`()
        assertThat(ThrottleBrakeImpl.motionState)
            .isEqualTo(ThrottleBrakePedal.ACTION_MOVE_FORWARD)
            .isEqualTo("forward")
    }
    @Test
    fun `backward motion state`() {
        `is moving backward`()
        assertThat(ThrottleBrakeImpl.motionState)
            .isEqualTo(ThrottleBrakePedal.ACTION_MOVE_BACKWARD)
            .isEqualTo("backward")
    }
    @Test
    ...
    fun `throttle forward`() {
        val ret = ThrottleBrakeImpl.throttle(ThrottleBrakePedal.ACTION_MOVE_FORWARD, -30)
    ...
    fun `throttle backward`() {
        val ret = ThrottleBrakeImpl.throttle(ThrottleBrakePedal.ACTION_MOVE_BACKWARD, 300)
        assertThat(ret).isEqualTo(EngineSystem.SUCCESS)
        assertThat(ThrottleBrakeImpl.motionState)
            .isEqualTo(actionMemProp?.getter?.call())
            .isEqualTo(ThrottleBrakePedal.ACTION_MOVE_BACKWARD)
    ...
    fun `client activate handbrake`() {
        ...
        assertThat(ThrottleBrakeImpl.motionState)
            .isEqualTo(actionMemProp?.getter?.call())
            .isEqualTo(ThrottleBrakePedal.ACTION_HANDBRAKE)
            .isEqualTo("handbrake")
        assertThat(valueMemProp?.getter?.call(ThrottleBrakeImpl)).isEqualTo(100)
    }
    @Test
